...
WandExport MagickBooleanType TJCompareImageCommand(ImageInfo *image_info,
...
{ \
  ...
  */
  ...
  /* XXX TODO TJ */
  size_t mdest_len = 2048;
  char* mdest = malloc(mdest_len);
  *metadata = mdest;
  /* XXX TODO TJ */
      ...
      if (image_info->verbose == MagickFalse)
        {
          switch (metric)
          {
            ...
            {
              (void) FormatLocaleString(mdest, mdest_len, "%g (%g)",QuantumRange*distortion,
                (double) distortion);
              if ((reconstruct_image->columns != image->columns) ||
                  (reconstruct_image->rows != image->rows))
                (void) FormatLocaleString(mdest, mdest_len, " @ %.20g,%.20g",(double)
            ...
            {
              (void) FormatLocaleString(mdest, mdest_len, "%g",distortion);
              if ((reconstruct_image->columns != image->columns) ||
                  (reconstruct_image->rows != image->rows))
                (void) FormatLocaleString(mdest, mdest_len, " @ %.20g,%.20g",(double)
                  difference_image->page.x,(double) difference_image->page.y);
            ...
            {
              (void) FormatLocaleString(mdest, mdest_len, "%g (%g, %g)",distortion,
      ...
      else
        {
          ...
          (void) FormatLocaleString(mdest, mdest_len, "Image: %s\n",image->filename);
          if ((reconstruct_image->columns != image->columns) ||
              (reconstruct_image->rows != image->rows))
            (void) FormatLocaleString(mdest, mdest_len, "Offset: %.20g,%.20g\n",(double)
              difference_image->page.x,(double) difference_image->page.y);
          (void) FormatLocaleString(mdest, mdest_len, "  Channel distortion: %s\n",
          ...
          switch (metric)
          {
            ...
            case PeakAbsoluteErrorMetric:
            {
              switch (image->colorspace)
              {
                ...
                case GRAYColorspace:
                {
                  (void) FormatLocaleString(mdest, mdest_len, "    gray: %g (%g)\n",
              ...
              (void) FormatLocaleString(mdest, mdest_len, "    all: %g (%g)\n",
            ...
            case PeakSignalToNoiseRatioMetric:
            {
              switch (image->colorspace)
              {
                ...
                default:
                {
                  (void) FormatLocaleString(mdest, mdest_len, "    red: %g\n",
                    channel_distortion[RedChannel]);
                  (void) FormatLocaleString(mdest, mdest_len, "    green: %g\n",
                    channel_distortion[GreenChannel]);
                  (void) FormatLocaleString(mdest, mdest_len, "    blue: %g\n",
                    channel_distortion[BlueChannel]);
                  if (image->matte != MagickFalse)
                    (void) FormatLocaleString(mdest, mdest_len, "    alpha: %g\n",
                      channel_distortion[OpacityChannel]);
                  break;
                }
                case CMYKColorspace:
                {
                  (void) FormatLocaleString(mdest, mdest_len, "    cyan: %g\n",
                    channel_distortion[CyanChannel]);
                  (void) FormatLocaleString(mdest, mdest_len, "    magenta: %g\n",
                    channel_distortion[MagentaChannel]);
                  (void) FormatLocaleString(mdest, mdest_len, "    yellow: %g\n",
                    channel_distortion[YellowChannel]);
                  (void) FormatLocaleString(mdest, mdest_len, "    black: %g\n",
                    channel_distortion[BlackChannel]);
                  if (image->matte != MagickFalse)
                    (void) FormatLocaleString(mdest, mdest_len, "    alpha: %g\n",
                      channel_distortion[OpacityChannel]);
                ...
                case GRAYColorspace:
                {
                  (void) FormatLocaleString(mdest, mdest_len, "    gray: %g\n",
                    channel_distortion[GrayChannel]);
                  if (image->matte != MagickFalse)
                    (void) FormatLocaleString(mdest, mdest_len, "    alpha: %g\n",
                      channel_distortion[OpacityChannel]);
              ...
              (void) FormatLocaleString(mdest, mdest_len, "    all: %g\n",
                channel_distortion[CompositeChannels]);
              break;
            }
            case MeanErrorPerPixelMetric:
            {
              (void) FormatLocaleString(mdest, mdest_len, "    %g (%g, %g)\n",
                channel_distortion[CompositeChannels],
                image->error.normalized_mean_error,
                image->error.normalized_maximum_error);
              break;
